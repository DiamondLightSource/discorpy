...
- As can be seen in :numref:`fig_33` (b), the slopes are significantly different between
  ...
  regenerated using these updated coefficients (:numref:`fig_35`).
    ...
    .. code-block:: python
        ...
        # Save output for checking
        losa.save_plot_image(output_base + "/horizontal_lines_regenerated.png", list_hor_lines,
                           height, width)
        losa.save_plot_image(output_base + "/vertical_lines_regenerated.png", list_ver_lines,
                           height, width)
